...
Classes:
    Rectangle: defines a rectangle by its width and height, makes area
        ...
        possible, it returns a string made up of a chosen symbol
        representing each unit length (corresponding by default to the
        character '#') and '\n' (to separate the rows) as necessary.
        Thus, making print() draw a rectangle made of chosen symbol.
        ...
        The symbol to be used in the user-friendly string
        representation is called print_symbol and it may be provided by
        the user as any type.
"""
...
class Rectangle(object):
    ...
    Description:
        ...
        string to represent the rectangle with a certain character or
        string (character '#' by default) as its unit element and '\n'
        to delimit the rows of characters that will make up a drawing
        of the rectangle if print is used. The canonical string
        representation method (__repr__) is also made available such
        that eval may recreate the Rectangle instance.
        __del__ is set such that a message is printed when an instance
        of Rectangle is deleted. The class stores the number of
        instances currently in memory in the public class attribute
        number_of_instances. The public class attribute print_symbol
        is available to be set by the user, or remain as '#' otherwise.
    """
    ...
    def bigger_or_equal(rect_1, rect_2):
        """
        Determine bigger rectangle and return it.

        Examples:
        >>> rect1 = Rectangle(1, 2)
        >>> rect2 = Rectangle(2, 3)
        >>> rect = Rectangle.bigger_or_equal(rect1, rect2); rect.area()
        6
        """
        for rect, rect_str in [(rect_1, "rect_1"), (rect_2, "rect_2")]:
            if not isinstance(rect, Rectangle):
                raise TypeError(f'{rect_str} must be an instance of '
                                'Rectangle')
    ...
    def __del__(self):
        ...
        """
        type(self).number_of_instances -= 1
        if not type(self).suppress_del_print:
            print("Bye rectangle...")
...
if __name__ == "__main__":
    ...
    file_path = "9-rectangle.py"
    test_file_path = "tests/test_rectangle_9.txt"
