#!/usr/bin/python3
"""
Module Name: task_02_verboselist.

Contains class VerboseList that extends the list class.
"""
from typing import SupportsIndex


class VerboseList(list):
    """Extend list class to print a notification when method called."""

    def append(self, item):
        """Add notification message to list append method."""
        super().append(item)
        print(f"Added [{item}] to the list.")

    def extend(self, iter):
        """Add notification message to list extend method."""
        super().extend(iter)
        print(f"Extended the list with [{len(iter)}] items.")

    def remove(self, value):
        """Add notification message to list remove method."""
        print(f"Removed [{value}] from the list.")
        super().remove(value)

    def pop(self, idx: SupportsIndex = -1):
        """Add notification message to list pop method."""
